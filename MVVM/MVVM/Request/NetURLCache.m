...
+ (void)active {
    ...
    //2.
    NetURLCache *cache = [[NetURLCache alloc] initWithMemoryCapacity:1<<M_MAX_FILE_SIZE
...
{
    NSLog(@"%@", [[request allHTTPHeaderFields] objectForKey:URLCACHE_CACHE_KEY]);
...
- (NSCachedURLResponse *)findCacheResponseInDisk:(NSURLRequest *)request {
    ...
    NSString *path = [NetURLCache responseFilePathForRequest:request RootPath:_cacheDirectory];
    ...
    if (cachedItem) {
        ...
        if ([NetURLCache networkAvailable]) {   //服务器可达，需要判断缓存是否超时，不可达直接返回缓存
        ...
        return [NetURLCache getCacheURLResponseWithData:[cachedItem cacheData] Request:request];
    }
    //------------------------------------------------------
...
- (NSCachedURLResponse *)findCacheItemInMemory:(NSURLRequest *)request {
    ...
    NSString *key = [NetURLCache getRequestMD5Key:request];
    ...
    if (![cacheItem isExpirat]) {
        ...
        return [NetURLCache getCacheURLResponseWithData:[cacheItem cacheData] Request:request];
    }
...
- (BOOL)requestCanCachable:(NSURLRequest *)request {
    ...
    [NetURLCache networkAvailable];
    
    
    NSLog(@"%lu", (unsigned long)request.cachePolicy);
